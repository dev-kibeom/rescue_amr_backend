...
export default function MonitorPage() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      if (robotRes.ok) {
        setRobots(await robotRes.json());
        setDbError(false);
      }

      if (survivorRes.ok) {
        const logs = await survivorRes.json();
        // 이름 있는 것 = 식별된 생존자, 없는 것 = 미식별
      ...
      <main className="grid-monitor">
            ...
            <div className="dash">
              <div className="dash-center">
                <div className="sub-header split">
                  ...
                  {dbError && <span style={{ color: "var(--red-light)", fontSize: "0.75rem" }}>⚠ DB 연결 없음</span>}
                </div>
                <div className="map-area">
                  <div className="map-svg-bg" />
                  {/* DB에서 받은 로봇 마커 */}
                  {robots.length === 0 && !dbError && (
        ...
        <section className="cell status-cell">
          ...
          <div className="status-dashboard">
            ...
            {dbError ? (
              <div style={{ color: "var(--red-light)", fontSize: "0.8rem", textAlign: "center", padding: "0.5rem 0" }}>
                <i className="ti ti-wifi-off" /> 백엔드 서버 연결 없음
              </div>
            ) : (
              <>
            ...
            <div className="ring-row" style={{ justifyContent: "center" }}>
              <Ring
                percent={robots.length > 0 ? Math.round((robots.filter(r => r.status === "SUCCESS").length / robots.length) * 100) : null}
                tone="rescue"
                label="탐사"
                sub="탐사 완료율"
                size={76}
