...
#   ./run_ares_vision.sh robot2 8003 /robot2/oakd/rgb/image_raw/compressed
#
# 환경변수로도 설정 가능:
#   WEBRTC_PORT=8002 WEBRTC_IMAGE_TOPIC=/robot1/... ./run_ares_vision.sh
...
# vision_ws가 있으면 source
if [ -f "$HOME/vision_ws/install/setup.bash" ]; then
  source "$HOME/vision_ws/install/setup.bash"
fi
...
if [ "$ROBOT_ID" = "robot2" ] || [ "$ROBOT_ID" = "TB_05" ]; then
  DEFAULT_DISCOVERY=";192.168.108.105:11811;"
  DEFAULT_TOPIC="/TB_05/oakd/rgb/image_raw/compressed"
else
